...
Sequence("Consent", "Welcome", "Start",  "AlienIntro", "GroupingIntro", "AlienReminder1", "GroupingNulu", "AlienReminder2", "GroupingGili", "nonsocial", "groupingoutfitintro", startsWith("grouping1"), "feedback1", "LearningIntro1", "LearningIntro2",
...
newTrial("AlienIntro",
noTopMargin(),
  newText("Nulus1", "Nulu wearing the black outfit")
  .css("font-size", "1.5em")
  .log()
  ,
  newText("Nulus2", "Nulu wearing the blue outfit")
  .css("font-size", "1.5em")
  .log()
  ,
  newText("Gilis1", "Gili wearing the black outfit")
  .css("font-size", "1.5em")
  .log()
  ,
  newText("Gilis2", "Gili wearing the blue outfit")
  .css("font-size", "1.5em")
  .log()
  ,
...
// Introducing the Grouping activity of aliens
...
//grouping 1: Find Nulus
newTrial("AlienReminder1",
noTopMargin(),
...
//Grouping activity of aliens
//Grouping Nulu
newTrial("GroupingNulu",
 noTopMargin(),
...
//Introducing the grouping activity of outfits
newTrial("nonsocial",
  noTopMargin(),
  
  newHtml("nonsocial.html")
    .log()
    .print()
  ,
  newVar("RT").global().set( v=> Date.now()),
...
//staring the grouping of outfits
/*
newTrial("OutfitReminder1",
  noTopMargin(),

  newText("<p> Now let's start by inviting an equal number of aliens in the blue outfit and in the black outfit! </p>")
  .css("font-size", "1.5em")
  .center()
  .print()
  .log()
...
newTrial("groupingoutfitintro",
    newVar("nblue",0).global(),
    newVar("nblack",0).global()
    //,
    //newButton("Start").center().print().wait()
)
.setOption("hideProgressBar", true) 

Template( "Grouping1.csv", row =>
    newTrial( "grouping1"+row.Item ,
        defaultImage.size(270,266)
        ,
        newCanvas("container", 650,300)
            .add("left at 0%",    0, newImage("black", row.ImageBlack) )
            .add("right at 100%", 0, newImage("blue",  row.ImageBlue)  )
            .print()
        ,
        newText("instruction", "Please select one delegation of aliens to invite by clicking on it!")
           .css("font-size", "1.5em")
           .print()
        ,
        newSelector("choice")
            .add( getImage("blue") , getImage("black") )
            .frame("solid 5px violet")
            //.once()
            .log()
            .wait()
            .test.selected( getImage("blue") )
            .success( getVar("nblue").set(v=>v+Number(row.NumberBlue)) )
            .failure( getVar("nblack").set(v=>v+Number(row.NumberBlack)) )
        ,
        newButton("Next")
            .size("font-size", "2em", "center")
            .print()
            .wait()
    )
    .setOption("hideProgressBar", true) 
...
newTrial("feedback1",
    getVar("nblue")
        .test.is( getVar("nblack") )
        .success( newText("good", "Congratulations! You kept the outfit colors evenly balanced, so our guests were happy and the party was a success! Let’s hope we continue to do so well!").print() .css("font-size", "1.2em") .log())
        .failure( newText("bad", "Oh no! There were more guests wearing one outfit than the other! The guests weren’t happy! Let’s hope we can do better from now on!").print() .css("font-size", "1.2em") .log()  )
    ,
...
newTrial("GroupingReminder2",
  noTopMargin(),

  newText("<p> Now let's invite an equal number of Nulus and Gilis in any outfit! </p>")
  .css("font-size", "1.5em")
  .center()
  .print()
  .log()
...
//Grouping of aliens
newTrial("grouping2intro",
    newVar("nnulu",0).global(),
    newVar("ngili",0).global()
    //,
    //newButton("Start").center().print().wait()
)
.setOption("hideProgressBar", true) 

Template( "Grouping2.csv", row =>
    newTrial( "grouping2"+row.Item ,
        defaultImage.size(270,266)
        ,
        newCanvas("container", 550,300)
            .add("left at 0%",    0, newImage("nulu", row.ImageNulu) )
            .add("right at 100%", 0, newImage("gili",  row.ImageGili)  )
            .print()
        ,
        newText("instruction", "Please choose a group of aliens by clicking the image!")
           .print()
        ,
        newSelector("choice")
            .add( getImage("gili") , getImage("nulu") )
            .frame("solid 5px violet")
            .log()
            .wait()
            .test.selected( getImage("gili") )
            .success( getVar("ngili").set(v=>v+Number(row.NumberGili)) )
            .failure( getVar("nnulu").set(v=>v+Number(row.NumberNulu)) )
        ,
        newButton("Next")
            .size("font-size", "2em", "center")
            .print()
...
newTrial("feedback2",
    getVar("ngili")
        .test.is( getVar("nnulu") )
        .success( newText("Congratulations! You kept the outfits evenly balanced so it looked very nice! The party was a great success and pictures look great! Hopefully your next party will go similarly well.").print() )
        .failure( newText("Oh no! There were more guests wearing one outfit than the other! It looked uneven and the party was a disaster! Hopefully your next party will go better.").print()   )
    ,
    newButton("Finish").print().wait()
)
.setOption("hideProgressBar", true) 
